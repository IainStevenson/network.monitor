...
namespace netmon.domain.tests.Unit.Orchestrators
{
    public class PingResponseModelReStorageOrchestratorTests : TestBase<PingResponseModelReStorageOrchestrator>
    {
        ...
        public override void Setup()
        {
            ...
            {
                _testFolder.Create();
                var items = Enumerable.Range(0, 5).Select(s => new PingResponseModel()).ToList();
            ...
            _fileSystemRepository = Substitute.For<IFileSystemRepository>();
        ...
        [Test]
        public void OnMoveFilesToObjectStorageItStoresNoFilesIfCancellation()
        {
            // Arange
            // note this is what teh mocked repository actually does here but we are isolating
            var filesFound = _testFolder.EnumerateFiles("*.json");
