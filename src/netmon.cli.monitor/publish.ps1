# forked with gratitude, from https://github.com/norcino/docker-dotnet-core-api-qnap-nas
#
properties {
	# Name of the container
	$containerName = "netmon.monitor.1"
	# Name of the image
	$imageName = "netmonmonitor"
    ...
    $srcDir = $baseDir
	# Solution file path
    $solution = Join-Path $baseDir "\netmon.sln"
	# Project directory, refers to the master project that will be published and executed
	$projectDir = Join-Path $srcDir "\netmon.cli.monitor"
	# Project file path
	$project = Join-Path  $projectDir "\netmon.cli.monitor.csproj"
